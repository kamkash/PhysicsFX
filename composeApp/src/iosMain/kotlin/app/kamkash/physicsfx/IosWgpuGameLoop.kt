...
class IosWgpuGameLoop : WgpuGameLoop {
    ...
    override fun start(surfaceHandle: Any?, width: Int, height: Int) {
        ...
        // surfaceHandle should be a COpaquePointer to CAMetalLayer
        val surfacePtr: COpaquePointer? =
                when (surfaceHandle) {
                    is COpaquePointer -> surfaceHandle
                    null -> {
                        println("WARNING: No surface handle provided for iOS")
                        null
                    }
                    else -> {
        ...
        // Initialize wgpu via C interop with the surface handle
        val initialized = wgpu_init(surfacePtr, width, height)
        if (!initialized) {
        ...
        gameLoopJob =
                gameScope.launch {
                    while (running && isActive) {
                        ...
                        // Frame pacing
                        val frameTimeElapsed =
                                platform.Foundation.NSDate().timeIntervalSince1970 - currentTime
